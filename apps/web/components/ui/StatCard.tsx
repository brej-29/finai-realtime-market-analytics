  ...
  return (
    <Card className="group relative overflow-hidden p-4 transition-colors hover:border-border-hover/20 sm:p-5">
      <div className="flex items-start justify-between">
        ...
        {icon && (
          <span className="text-muted transition-colors group-hover:text-brand-light">{icon}</span>
        )}
      </div>
      <p className={cn("mt-3 text-2xl font-semibold tabular-nums", toneClasses[tone])}>
        {loading ? (
      ...
      {helper && <div className="mt-1.5 text-xs text-muted">{helper}</div>}
      <motion.div
